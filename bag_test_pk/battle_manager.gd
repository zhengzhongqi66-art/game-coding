...
func _setup_ui():
	...
	log_panel.custom_minimum_size = Vector2(180, 120)
...
func start_battle(player_items: Array):
	current_state = BattleState.FIGHTING

	player_unit.update_stats_from_inventory(player_items)

	enemy_unit.max_hp = 80 + randi() % 40
	enemy_unit.current_hp = enemy_unit.max_hp
	enemy_unit.base_attack = 8 + randi() % 5
	enemy_unit.base_defense = 3 + randi() % 3
	...
	_add_log("敌人 HP=%d ATK=%d" % [enemy_unit.max_hp, enemy_unit.base_attack])
...
func _process(delta):
	...
	if enemy_attack_timer >= enemy_unit.attack_speed:
		enemy_attack_timer = 0.0
		_perform_attack(enemy_unit, player_unit, "敌人")
...
func _perform_attack(attacker, defender, attacker_name: String):
	...
	if died:
		...
		var winner = "玩家" if defender == enemy_unit else "敌人"
